...
BRCA_noBatch <- readRDS("BRCA/BRCA_noBatch_v2.rds")
...
for(x in pathway_name){
  print(x)
  start <- proc.time()[3]
  test2 <- MFA_pathway_all(omics_data = BRCA_noBatch,
                           base_ids = 1:506,
                           supp_ids = NULL,
                           pathway_name = x,
  ...
  save(list=ls()[grepl("output_", ls())], file="MFA_BRCA_Outputs_May19.RData")  
}
